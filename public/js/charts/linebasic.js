$(document).ready(function(){
	...
	$('form#createLineBasicChart').submit(function(e){
		e.preventDefault();
		//collect line columns

		if($('ul#sortable2').find('li').length == 0){
        	new PNotify({
			    title: 'Error!',
		...
		$('ul#sortable2').find('li').each(function(){
			lineColumns[$(this).find('span.columnText').text()] = $(this).data('id');
		});
		...
		var params = $('form#createLineBasicChart').serializeArray();
		params.push({name: 'lineColumns', value: JSON.stringify(lineColumns) });
	...
	$('form#createLineBasicChart').on('click','a.addLineColumnBtn',function(){
	...
	$('form#createLineBasicChart').on('click','a.removeLineColumnBtn',function(){
	...
	$('div.chartWrapperDiv').on('click','a.chartViewSaveBtn',function(){
		...
		$('form#createLineBasicChart').trigger('submit');
