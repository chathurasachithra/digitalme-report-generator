$(document).ready(function(){
    ...
    $("#sortable1, #sortable4, #sortable3 ").sortable({
	...
	$('form#createChart').submit(function(e){
		...
		if($('ul#sortable3').find('li').length == 0){
        	new PNotify({
			    title: 'Error!',
			    text: 'You have to select one column for the Pie Value Column',
			    type: 'error'
			});
			return false;
		}
		...
		if($('ul#sortable4').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select one column for the Pie Name Column',
				type: 'error'
			});
			return false;
		}
		var pieNameColumn = $(this).find('ul#sortable4').find('li').data('id');

		var pieValueColumn = {};
		$('ul#sortable3').find('li').each(function(){
			pieValueColumn[$(this).find('span.columnText').text()] = $(this).data('id');
		});
		...
		params.push({name: 'pieNameColumn', value: pieNameColumn });
		params.push({name: 'pieValueColumn', value: JSON.stringify(pieValueColumn) });
